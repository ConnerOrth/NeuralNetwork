...
namespace NeuralNetwork
{
    public class NeuralNetwork
    {
        ...
        // ----------------------------------------------------------------------------------------
        public double[] ComputeResult(double[] xValues) => ComputeOutputs(xValues);
        ...
        public void Train(double[][] trainData, int maxEpochs, double learnRate, double momentum,
          double weightDecay, double meanSquaredErrorStopCondition = 0.020)
        {
            ...
            while (epoch < maxEpochs)
            {
                ...
                if (mse < meanSquaredErrorStopCondition) break;
                ...
                if (epoch % (maxEpochs/4) == 0) Console.WriteLine($"Currently at Epoch {epoch} of {maxEpochs}. Accuracy : {Accuracy(trainData).ToString("F4")}");
                ++epoch;
            }
        } // Train

        public void TrainParallel(double[][] trainData, int maxEpochs, double learnRate, double momentum,
          double weightDecay, double meanSquaredErrorStopCondition = 0.020)
        {
            // train a back-prop style NN classifier using learning rate and momentum
            // weight decay reduces the magnitude of a weight value over time unless that value
